...
class NoDependenciesTestCase(SeriesDependencyBase):
    def setUp(self):
        self.project = utils.create_project()
        utils.create_state()

    def test_no_dependencies_when_dependencies_disabled(self):
        ...
        self.assertNotIn(series2, series1.dependents.all())
        self.assertNotIn(series1, series2.dependencies.all())


class SeriesDependencyTestCase(SeriesDependencyBase):
    def setUp(self):
        self.project = utils.create_project(parse_dependencies=True)
        utils.create_state()
